...
class gamestate():
    ...
    def addBetsToPot(self):
        ...
        betsy = self.getBets()
        #setup a list of payers and a list of bets:
        betina = []
        payersi = []
        
        #iterate through every bet
        for kaka in betsy.keys():
            
            #make sure not to include best
            if kaka != 'best':
                bet = betsy[kaka]
                
                #store player who payed this bet
                if kaka in self.getRoundPlayers():
                    payersi.append(kaka)
                
                #exclude double bets:
                if bet not in betina:
                    
                    #consider only bets from players still in round:
                    if kaka in self.getRoundPlayers():
                        betina.append(bet)
        ...
        allin = False
        for every in self.levelplayers[self.getLevel()]:
            if every in self.getRoundPlayers():
                if every not in payersi:
                    allin = True
        if allin == True:
            self.sidelevel += 1
        
        if len(betina) == 1:
            if self.getLevel() in self.levelpot.keys():
                self.levelpot[self.getLevel()] = self.levelpot[self.getLevel()] + self.getBetsSum()
            else:
                self.levelpot[self.getLevel()] = self.getBetsSum()
            self.levelplayers[self.getLevel()] = payersi
            self.bets = {'best':0}
        
        #iterate as long as there are some levels to examine
        else:
            while len(betina) != 0:
                ...
                #LOWEST LEVEL:          
                lowestlimit = betina.pop(0)
                
                levelsum = 0
                
                #produce next level if len(betina) !=1:
                if len(betina) != 0:
                    betina[0]= betina[0] - lowestlimit
                    self.sidelevel += 1                
                
                for aplayer in betsy.keys():
                    if aplayer != 'best':
                        
                        abet = self.bets[aplayer]
                        
                                                
                        #if bet is smaller than lowestlimit:
                        #it means that player folded
                        #or that he doesn't belong to this level
                        ...
                        if abet < lowestlimit:
                            ...
                            #remove his bet
                            betsy.pop(aplayer)
                            #remove him from payers:
                            if aplayer in helperlist:
                                
                                helperlist.remove(aplayer)
                        
                        #if bet is bigger than lowestlimit:                       
                        if abet >= lowestlimit:
                            levelsum += lowestlimit
                            betsy[aplayer]=betsy[aplayer]-lowestlimit
                        
                if level in self.levelpot.keys():            
                    self.levelpot[level]=self.levelpot[level]+ levelsum
                else:
                    self.levelpot[level]=levelsum
                self.levelplayers[level]=helperlist
    ...
    def getPot(self):
        suam = 0
        for every in self.levelpot.keys():
            suam += self.levelpot[every]
        return suam  
    ...
    def displayPot(self):
        print
        if self.getLevel() ==0:
            print 'The pot is %s' %(self.levelpot[0])
        else:
    ...
    def play(self):
        ...
        if skip != True:
            ...
            print 'Player %s won and gets %s chips' %(self.winners[0].getName(),self.getPot())
            self.winners[0].addToMoney(self.getPot())
...
class player():
    ...
    def update(self,gamestate):
        ...
        if self.getAllIn() != True:
            if tocall > self.getMoney():
                if 4*blinds > self.getMoney():
        ...
        # #POSSIBLE GAMESTAGES:              
        #=======================================================================
        if self.getAllIn() != True and folded != True:
            ...
            #turn
                ...
                if random.random() > 0.6:
                    ...
                    #evaluate hand:
                    chancehands = cards.findhands(sumofknowncards)
                    
                    skiper = False
                    if chancehands[-1].getType() in ('flush','full','four','straightflush', 'royalflush'):
                        self.allIn(gamestate)
                        skiper = True
                    if skiper != True:                    
                    
                        if len(chancehands) > 1:
                            if gamestate.getPlayerBets(self)== 0:
